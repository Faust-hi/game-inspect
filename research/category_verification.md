...
> **Из чего состоит 1794.** Планировщик использует только опубликованные
> записи: **802** пакета с кодами `<метод>.<этап>` (по 7 на метод), созданные
> `evidence_catalog.sync_work_packages`. Остальные **992** — черновики с кодами
> `WP_<метод>_<тип>` из `pack_loader`, они в расчёт не попадают. Смешивать их
> нельзя: у черновиков роли не из словаря сценариев команды и трудозатраты
> метода делятся поровну на 8 типов пакета.
...
|---|---|---|---|---|---|
| solo | **208,99** | 190,45 | 139 | 21 | 9 |
| small_2_5 | **208,99** | 121,78 | 139 | 21 | 9 |
| mid_6_15 | **208,99** | 55,87 | 139 | 21 | 9 |
| large_16_plus | **208,99** | 27,80 | 139 | 21 | 9 |

**Инвариант подтверждён живьём:** `effort` (человеко-дни) **не меняется** от
размера команды (208,99 при любом размере); календарь **падает**
190,45 → 27,80 дней (solo → large, сжатие ×6,9). Критический путь — 9 задач:

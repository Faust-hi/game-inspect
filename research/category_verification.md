...
**Полная база на момент сессии 3 (прямой запрос):**

```
evidence_claims 2562 (published 2539, draft 23)   evidence_sources 936 (published 936)
conflicts 456    dependency_edges 927 (mandatory 164)   game_cases 155   case_evidence 375
...
> **АКТУАЛИЗАЦИЯ (2026-09-11, аудит-сессия).** Блок выше — снимок сессии 3.
> С тех пор сиды изменились, поэтому счётчики в таблицах 1.4 / 1.6 / 1.7 и в
> сводной таблице плана (стр. «1.4», «1.6», «1.7») относятся к **тому** снимку,
> а не к текущему состоянию. Перепроверенное состояние production-БД на конец
> аудита:
>
> ```
> evidence_claims 2564 (published 2540, draft 24)   evidence_sources 936 (published 936)
> conflicts 456    dependency_edges 930 (mandatory 165)   game_cases 155   case_evidence 375
> methods 124      game_functions 40   engines 7   engine_tools 71   technology_nodes 222
> method_engine_links 475   work_packages 1794   hardware_cpu 51   hardware_gpu 79
> claims по basis: documented 1581 · case_evidence 244 · unknown 263 · derived 208 ·
>                  expert_estimate 164 · measured 104
> ```
>
> Разница со снимком сессии 3: **+1 инструмент** (`ue_networking` — он объявлен в
> сидах, `backend/app/seed/engines_data.py:166`, и на него ссылаются два метода),
> **+2 связки** метод↔инструмент, **+1 technology node**, **+3 ребра графа**,
> **+2 утверждения** (+1 published). Обязательные циклы — по-прежнему **0**.
>
> Как перепроверено: прямым запросом к `backend/gamedev_dss.db`; сборкой с нуля
> (`tools/check_seed_convergence.py` — совпали все 18 таблиц между сборкой с нуля,
> +3 полными сидами и +3 стартами приложения) и `tools/audit_evidence.py`
> (обязательные нули на месте). Числа в разделах 1.1–2.10, включая проценты
> покрытия, намеренно **не** переписаны: они относятся к снимку сессии 3 и
...
| 1.5 Конфликты | `conflicts` | 456 строк, 7 типов | — |
...
## 1.5 Конфликты зависимостей (`conflicts`, 456 строк, 7 типов)

**Распределение по типам (прямой запрос):**

| Тип связи | Строк | Смысл |
|---|---|---|
| `complement` | 181 | Взаимодополнение (взаимное) |
...
alpha 7. Роли: engineering 306, technical_art 124, qa 124, production 124,
design 124.

> **Одна семья вместо двух (N2, 2026-09-11).** Раньше в таблице было 1794
> записи: 802 опубликованных `<метод>.<этап>` из `evidence_catalog` и 992
> черновика `WP_<метод>_<тип>` из `pack_loader`. Это были две независимые
> оценки одной работы, а не одна в двух масштабах: корреляция итого по методу
> 0,449, отношение от 0,49 до 19,33, у 44 методов из 124 расхождение больше
> двукратного. Формула давала всего 12 различных итогов на 124 метода (разброс
> 3,02×) и не отличала конвейер от точечной правки; курируемая оценка — 28
> итогов (разброс 86,7×). Публиковать вторую семью как есть было нельзя: она
> не проходила через планировщик и не несла `min_days`, `late_factor`,
> `parallelizable` и `dependency_codes`.
>
> Теперь величина берётся из паков (`effort_person_days`, курируемая оценка),
> а распределение по фазам — из формулы (доли `implementation_cost` и
> `complexity`). Итог — производная величина с записанным правилом: 2435,0
> чел-дней P50 вместо 1081,82. Подробнее —
> `research/n2-recommendation-2026-09-11.md`.
...
предусловиями до **19 методов** (126 задач, 0 неразрешённых зависимостей).

| Команда | `effort.p50` (чел-дни) | `calendar.p50` (дни) | задач | методов | критических |
|---|---|---|---|---|---|
| solo | **702,00** | 678,33 | 126 | 19 | 9 |
| small_2_5 | **702,00** | 453,54 | 126 | 19 | 9 |
| mid_6_15 | **702,00** | 222,69 | 126 | 19 | 9 |
| large_16_plus | **702,00** | 116,31 | 126 | 19 | 9 |

**Инвариант подтверждён живьём:** `effort` (человеко-дни) **не меняется** от
размера команды (702,00 при любом размере); календарь **падает**
678,33 → 116,31 дней (solo → large, сжатие ×5,8). Критический путь — 9 задач:
`async_loading_pipeline` (design→integration) и затем
`world_partition_streaming`
(design→feasibility→integration→content→optimization→qa→release). Все девять
помечены `critical`, что совпадает с множеством `critical_path`.

> **Почему путь стал длиннее (было 7 задач, 190,12 / 116,58 / 52,59 / 25,12).**
> `work_packages.dependency_codes` у всех 802 опубликованных пакетов были
> пустыми: проход `sync_work_packages` создавал строки до загрузки паков, а
> именно паки объявляют часть связей «метод требует метод», и поле больше не
> обновлялось. Планировщик поэтому не ставил первый пакет метода после
> интеграционного пакета его предусловия, и зависимость
> `world_partition_streaming → async_loading_pipeline` в календаре не
> проявлялась. Дефект устранён 2026-09-11; предусловия есть у 64 методов
> (414 пакетов).
...
> **Почему состав корзины сократился с 21 метода до 19.** Сняты три связи,
> где метод был объявлен обязательным предусловием того, с чем он
> несовместим: `motion_matching` → `animation_lod_budget` (между ними
> `hard_conflict`), `sdf_global_illumination` → `hardware_raytraced_gi`,
> `full_path_tracing_pipeline` → `selective_ray_traced_effects`. Движок
> применял обе связи и выдавал корзине два взаимоисключающих сообщения.
> Первая из них и убрала из плана `animation_lod_budget` вместе с его
> собственной зависимостью `gpu_skinning_compute`.

**Сверка с планом 2.10: ВЫПОЛНЕНО.** Инвариант «team size ≠ person-days»
подтверждён (мои живые числа: 702,00 чел-дней при любом размере команды;
календарь 678,33 → 116,31).
...
| 1.5 | Конфликты (456, 7 типов) | ✅ | ✅ | — | ✅ | **ВЫПОЛНЕНО** |

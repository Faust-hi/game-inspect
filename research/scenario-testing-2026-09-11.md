...
- **«Сверка с практикой» пуста:** ни один кейс каталога не сопоставлен с корзиной (сопоставление идёт только по коду метода, а часть доказательств привязана к функции).
...
- Оборудование: CPU-индекс 0.35 (класс 1, ориентир Core i3-8100), GPU-индекс 0.35 (класс 5, ориентир None), RAM 12.1 ГБ, VRAM 7.0 ГБ; узкое место — GPU (растеризация).
...
- **Оценка оборудования не выдана:** В каталоге нет GPU с подтверждённой поддержкой выбранного API и апскейлера.
- **Ориентир по видеокарте не выдан:** требуемый GPU-индекс 0.35 не покрыт каталогом (или не подтверждена обязательная аппаратная возможность), поэтому итоговая рекомендация пуста.
- **Выход за область применимости модели:** Количественный прогноз доступен только для Windows/Linux ПК: для платформ (ps4, xbox_one) совместимый аппаратный прогноз не обещается.
- **«Сверка с практикой» пуста:** ни один кейс каталога не сопоставлен с корзиной (сопоставление идёт только по коду метода, а часть доказательств привязана к функции).
- **FPS не моделируется:** целевые 60.0 FPS остаются статусом `not_modeled`; сравнить с фактической частотой кадров игры нельзя по устройству модели.

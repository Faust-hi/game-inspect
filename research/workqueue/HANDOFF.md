...
Дата последнего обновления: **2026-09-11 (сессия 4)**. Проект: локальная DSS для
...
# СЕССИЯ 4 — фиксация снимка исследований

Все исследовательские материалы **закоммичены и помечены тегом**, чтобы можно было
безопасно начинать интеграцию и калибровку проекта.

| Что | Значение |
|---|---|
| Тег | `research-snapshot-2026-09-11` (коммит `bce3a88`) |
| Коммиты | `36d8400` (gitignore) → `8b78595` (backend) → `b1f0377` (frontend) → `83b3053` (research) → `bce3a88` (манифест) |
| Манифест | `research/RESEARCH_SNAPSHOT.md` — инвентарь 93 файлов + SHA256 |
| Контрольные суммы | `research/RESEARCH_SNAPSHOT.sha256` (93/93 OK) |
| Тесты | 91 passed (`.dss-venv`) |
| Целостность git | `git fsck` — 0 ошибок (починено объектное хранилище, см. §7 манифеста) |

**Починка `.git`.** Объектное хранилище было повреждено (отсутствовали tree/commit-объекты,
плюс сломанные ref-ы Codex `refs/codex/turn-diffs/*`). Объекты восстановлены из `origin`
(remote `HEAD` совпадал с локальным `main` = `e04a4c33`); сломанные ref-ы вынесены в
`.git-safety-backup/git-20260911-110758/refs-backup/`. Форензическая копия исходного `.git`
сохранена в `.git-safety-backup/` (в git не входит).

**Что НЕ входит в git (осознанно).** `research/_mp/` (466 МБ внешних первоисточников),
`research/_verify_cache/` (производный кэш), `tmp/`, `*.db`, локальные кэши — всё это
воспроизводимо (см. §4 манифеста) и исключено через `.gitignore`.

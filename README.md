...
> Сборка PDF требует `reportlab`, и он намеренно не входит в
> `backend/requirements.lock`: приложению он не нужен, зависимость есть только у
> инструмента отчёта. Установите его в тот же интерпретатор, которым запускаете
> сборку:
>
> ```bash
> ./.venv/Scripts/python.exe -m pip install -r backend/requirements-report.txt
> ```
>
> Markdown-часть собирается и без `reportlab`; без него инструмент сообщает об
> этом явно и не пытается создать PDF.

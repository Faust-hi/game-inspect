...
## Точность оценки и её границы

Каталог: 124 метода реализации, 40 игровых функций, 7 движков, 51 CPU и 79 GPU;
2564 утверждения (2540 опубликовано) и 936 источников; 440 связей между
решениями (конфликты, зависимости, усиления, альтернативы, пересечения) и
914 рёбер графа зависимостей, из которых 474 — технологические (инструменты
движков, пакеты, runtime API).

Сверка ведётся с системными требованиями 25 реальных игр (2005–2025).

**Что оценка даёт:** определяющие техники — 44 из 52 ожидаемых (85 %), 11 из них
на первом месте; узкое место — 8 из 8 там, где издатель называет его прямо;
порядок величины и направление влияния.

**Чего оценка не даёт:** конкретную модель железа и объём сборки. Достижимая
точность — **±1 класс железа, ±~50 % по объёму**. Проценты расхождения по
компонентам (медианы: GPU +62.5 %, CPU −15.3 %, RAM −0.8 %, VRAM +35.0 %) — это
медиана **бимодального** распределения (разброс по GPU от −49 % до +1450 %), а
не точность предсказания.

**Почему так.** Эталон — паспортные требования издателя, а не замер: издатели
публикуют разные ступени (минимум или рекомендации), часть игр поднимала
требования патчами, а 8 сценариев из 25 вообще не сопоставить — эталон
ссылается на карты вне каталога (2013–2025), ещё у 5 нет рекомендованной
ступени. Разбор вкладов — в
`report/research-docs/wave10-attribution-2026-09-12.md`.

> Тесты и инварианты в `backend/tests/` держат **согласованность модели** —
> направление зависимостей, отсутствие двойного учёта, воспроизводимость
> расчёта. Они **не** являются доказательством точности оценки: сверка с
> реальностью — отдельный стенд, и её результат приведён выше.
...
├── tools/                    # проверки доказательной базы, отчёты, запуск Linux
│   ├── verify_sources.py     # проверка источников (сеть или офлайн, --packs-only)
│   ├── audit_evidence.py     # аудит покрытия утверждений
│   ├── check_research_invariants.py  # сверка чисел research/ с движком
│   ├── generate_research_report.py   # сборка отчёта (Markdown + PDF)
...
├── research/                 # CORE-документы исследования и паки с источниками
├── report/                   # архив: отчёты волн, стенды, первоисточники (не в git)
│   ├── PRESENTATION-KIT.md   # материалы для отчёта и защиты
│   ├── research-docs/        # отчёты волн 1–10 и разборы дефектов
│   └── artifacts/            # копии БД по волнам, собранные PDF
├── tmp/                      # стенды и зонды (не в git)
...
`output/pdf/game-development-dss-study.pdf` (каталог создаётся при сборке), а
также построчную матрицу покрытия в `research/catalog-coverage-matrix.md`. Если
рабочая база ещё не прошла новую миграцию, отчёт честно помечает её как
legacy-снимок и показывает ожидаемый seed-каталог вместо выдуманных чисел.

Собранные ранее артефакты и материалы исследования лежат в `report/`:
отчёты волн и разборы дефектов — `report/research-docs/`, собранные PDF и копии
БД по волнам — `report/artifacts/`, первоисточники — `report/sources/`. Сводная
опора для доклада — `report/PRESENTATION-KIT.md`.
...
адресу `/api/docs`. Административные маршруты находятся под `/api/admin`: сводка
(`/overview`), проверка целостности (`/validate`), заполнение демоданными
(`/seed`), методы и их статусы, сущности и журнал публикаций (`/entities`,
`/publication-log`), связи методов с инструментами (`/links`) и импорт CSV/JSON
(`/import/{entity}`). Доступ к ним рассчитан на локальный режим приложения.
...
Состав на 2026-09-12: backend — 89 обязательных и 76 подробных (165 всего),
frontend — 33, браузерных сценариев — 2. Обычный прогон backend занимает около
16 секунд, полный — около 27.

Стенд сверки с 25 профилями реальных игр живёт локально в `tmp/` и в
репозиторий не входит (каталог исключён из git). Он сравнивает вывод движка с
системными требованиями игр и считается источником чисел в разделе
«Точность оценки и её границы».

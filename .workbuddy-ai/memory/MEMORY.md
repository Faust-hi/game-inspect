...
- `cd backend && ../.venv/Scripts/python.exe -m pytest -q` — **63 теста**, 9 файлов
  критических свойств (сокращено с 452 коммитом 551ae62, затем 61 → 58 отбором
  по последствиям ошибки; +1 на неизвестный путь API, +4 на признак независимости
  от инструмента движка).
- Полезность набора подтверждена мутациями: ошибка вносится в продукт,
  ...
  применим только к зафиксированным файлам — при незафиксированных правках
  сначала делать резервные копии, иначе откат уничтожит работу.
...
Управляется Alembic (`backend/alembic/`); головная ревизия — `b2indep01`
(`methods.engine_tool_independent`). Старые базы распознаются в `db_migrate.py`
по маркерным столбцам (`LEGACY_MARKER_CHAIN`, порядок = порядок миграций:
штампуется самый свежий присутствующий маркер) и докатываются до head.
...
- Признак `engine_tool_independent` (по умолчанию False): решение реализуется
  своими средствами и не опирается на встроенный инструмент движка, поэтому
  пустой `engine_support` — не пробел данных. 13 таких методов перечислены
  в `methods_data.ENGINE_TOOL_INDEPENDENT`; синхронизация доносит признак
  до существующих баз и не стирает значение, поставленное вручную.
...
Реализованы все разделы плана 1–9 и 11. Наполнение MVP: 40 функций, 124 метода,

"""Регрессия классовых ориентиров ±1 класс — НЕ доказательство точности ±70%.

Цель проекта — относительная ошибка требуемой производительности CPU/GPU
≤70% (см. docs/calibration-protocol.md). Процент по произвольному индексу
каталога реальной игровой ошибкой не называется, поэтому эти тесты не
утверждают проверку процентной точности и не используются для настройки
коэффициентов.

Ниже — точечные регрессии на кейсах, где официальные требования имеют
проверенные цели 1080p/60 и точные якоря в каталоге: если данные каталога

...
def level_unspecified(value: str | None) -> bool:
    ...
    `unknown` — разрешённое значение анкеты: оно означает «источник не сообщает»,
    а не отдельный уровень нагрузки. Численно такому входу даётся **нейтральная**
    оценка (см. `_LEVEL_FALLBACK`), и это объявлено в пробелах модели («вклад
    взят по нейтральному уровню»). Без такой оговорки молчаливое совпадение
    нейтральной оценки с `medium` читалось бы как измеренная величина.
    """
...
#: Численная оценка уровня, когда точное значение не указано. `unknown` получает
#: нейтральную оценку — ту же, что `medium`. Это **объявленное допущение** (см.
#: `_modeling_gaps`: «вклад взят по нейтральному уровню»), а не измерение.
...
def _effective_count(value: int | None, level: str, field: str) -> float:
    ...
    объектов заметна, а между 1 000 000 и 1 010 000 — нет.

    Уровень и число — **альтернативные** входы одной шкалы, а не взаимозаменяемые
    величины: уровню соответствует реперная точка (`_LEVEL_FALLBACK`), а точное
    число считается по непрерывной логарифмической кривой. Поэтому одному и тому
    же ярлыку может соответствовать диапазон: «low» — это 0.25, тогда как 499
    объектов дают 0.04, а 4 999 — 0.33. Раньше докстрока утверждала, что шкалы
    совпадают, и это не соответствовало расчёту.
...
class HardwareEstimateOut(BaseModel):
    ...
    #: Класс подобранной карты/процессора. None — подходящей записи в каталоге
    #: нет. Раньше сюда подставлялось 5 («сильнейший класс»), и экран показывал
    #: «5 из 5» рядом с сообщением, что конфигурация не найдена.
    gpu_class: int | None = None
    cpu_class: int | None = None

...
METHODS: list[dict] = [
    ...
    M("world_origin_shifting", "Сдвиг начала координат (origin rebasing)",
      ...
      limitations=["Требует пересмотра всей работы с координатами",
                   "Решает точность больших координат, а не стоимость кадра"],
    ...
    M("gpu_compute_culling", "Отсечение объектов на GPU (compute)",
      ...
      limitations=["Нужен GPU с поддержкой вычислительных шейдеров",
                   "Само отсечение стоит отдельного вычислительного прохода: экономия "
                   "возникает на невидимой геометрии, а не на вычислениях"],
      requires_prototype=True,
    ...
    M("gpu_procedural_placement", "Процедурное размещение на GPU",
      ...
      limitations=["Требует вычислительных шейдеров",
                   ...
                   "и временные буферы занимают память"],
      requires_prototype=True,
    ...
    M("particle_pooling", "Пулинг систем частиц",
      ...
      limitations=["Не снижает среднюю нагрузку",
                   "Пул удерживает объекты: память под него остаётся занятой"],
    ...
    M("multithreaded_physics_jobs", "Многопоточная физика на системе задач",
      ...
      limitations=["Эффект зависит от числа доступных ядер",
                   "Меняет критический путь, а не суммарную работу шага: отдельной скидки физики нет"],
      requires_prototype=True,
    ...
    M("animation_compression", "Сжатие анимационных данных",
      ...
      limitations=["Требует подбора порогов по ассетам",
                   "Сжимается анимационный набор, а не геометрия сцены; декодирование "
                   "добавляет работу анимационной подсистеме"],
    ...
    M("time_sliced_pathfinding", "Поиск пути с распределением по кадрам",
      ...
      limitations=["Не снижает суммарную нагрузку",
                   "Работа переносится на следующие кадры: ответ приходит позже"],
    ...
    M("gerstner_fft_water", "Волны Герстнера и FFT на GPU",
      ...
      limitations=["Требует вычислительных шейдеров для FFT",
                   "Это визуальная поверхность: физическую симуляцию воды она не заменяет"],
      requires_prototype=True,
    ...
    M("deferred_forward_plus_choice", "Выбор архитектуры рендера (deferred / forward+)",
      ...
      limitations=["Выбор зависит от числа источников света и прозрачности",
                   "Стоимость ветвей различается по знаку: без указания выбранной ветви эффект не начисляется"],
      requires_prototype=True,
...
EXTRA_METHODS: list[dict] = [
    ...
    M("skeletal_2d_deform", "Скелетная 2D-анимация вместо покадровой",
      ...
      limitations=["Стиль «ломаной» мультипликации сложнее воспроизвести",
                   "Атлас полезен для батчинга, но не обязателен: скелетная анимация "
                   "работает и без него"],
      requires_prototype=True,
    ...
    M("pso_precaching_warmup", "Предкомпиляция и прогрев PSO",
      ...
      limitations=["Не лечит плохую оптимизацию сцены", "Требует прогона всего контента",
                   "Устраняет паузы компиляции, а не стоимость каждого кадра"],
      application_steps=[
    ...
    M("splitscreen_render_budget", "Бюджетирование split-screen",
      ...
      limitations=["На base-консолях только 30fps", "Требует отдельных пресетов",
                   "Число видов задаётся анкетой: подготовка рендера и геометрия уже "
                   "масштабируются им, бюджет только ограничивает стоимость каждого вида"],
      application_steps=[
    ...
    M("neural_texture_compression", "Нейросжатие текстур",
      ...
      limitations=["Без нейроускорения выгода частичная", "Требует пережатия всех наборов",
                   "Режим on-load экономит хранение, но не гарантирует меньшую резидентную "
                   "видеопамять; режим on-sample добавляет вычисления при обращении",
                   "Универсальной экономии затенения у метода нет"],
      application_steps=[
    ...
    M("planar_reflection_budget", "Бюджет плоских отражений",
      ...
      limitations=["Больше одного источника — только по веской причине",
                   "Ограниченное отражение дешевле неограниченного, но дороже отсутствия отражений"],
      application_steps=[
    ...
    M("normal_bake_retopology_pipeline", "Запекание нормалей и ретопология",
      ...
      limitations=["Не спасает плохую топологию анимаций",
                   "Ретопология сокращает геометрию, но нормали добавляют текстуры и выборку "
                   "в затенении: одновременной экономии по всем ресурсам нет"],
      application_steps=[
    ...
    M("build_size_startup_budgets", "Бюджеты размера сборки и старта",
      ...
      limitations=["Не ускоряет сам кадр, только вход в игру",
                   "Сам бюджет память не освобождает: эффект появляется после перекладки ассетов"],
      application_steps=[
    ...
    M("composition_bootstrap_architecture", "Композиционный каркас проекта",
      ...
      limitations=["Для мини-прототипа избыточно", "Требует дисциплины данных",
                   "Относится к старту и связности систем: стоимость кадра не уменьшает"],
      application_steps=[
    ...
    # =====================================================================
    # Трассировка лучей, меш-шейдеры, процедурный мир
    # =====================================================================
    M("full_path_tracing_pipeline", "Трассировка пути как основное освещение",
      "path_tracing",
      summary="Весь свет кадра считается трассировкой пути: единый проход вместо набора растровых "
              "приближений теней, отражений и переотражённого света.",
      description="Вместо раздельного растрового освещения применяется единая схема интегрирования: "
                  "каждый пиксель собирает путь луча до источника. Исключает дублирование "
                  "приближений, но стоимость полностью переходит в RT-бюджет и напрямую зависит "
                  "от разрешения и числа выборок на пиксель.",
      problem="Комбинирование растровых approximаций даёт несогласованную картину: тени, отражения "
              "и переотражённый свет считаются разными методами и конфликтуют друг с другом.",
      level="architecture", recommended_stage="preproduction", late_cost="critical",
      impact_gpu=2, impact_vram=1, impact_cpu=1,
      quality_impact=2, performance_gain=0.1, implementation_cost=5, complexity=5, confidence=0.7,
      applicable_platforms=["pc_windows", "ps5", "xbox_series"],
      requires_features=["path_tracing"],
      requires_hw_features=["Hardware Ray Tracing"],
      pros=["Физически согласованная картина без стыков между методами", "Корректные мягкие тени и многократные отражения"],
      cons=["Кратно дороже растрового освещения", "База оборудования ограничена RT-ядрами"],
      limitations=["Без аппаратной трассировки и денойзинга неприменимо"],
      requires_prototype=True,
      application_steps=[
          "Определить целевое разрешение и число выборок на пиксель.",
          "Подключить денойзер и проверить стабильность на движущейся камере.",
          "Заменить растровые источники освещения трассировочными по одному проходу.",
          "Замерять стоимость RT-прохода на минимальной поддерживаемой видеокарте.",
      ],
      verification_method="Замер времени RT-прохода и сравнение качества с растровым освещением.",
      verification_tools=["RenderDoc", "Unreal Insights", "NVIDIA Nsight Graphics"],
      source_key="WIKI_PATH_TRACING"),

    M("path_tracing_sample_denoiser_budget", "Бюджет выборок и денойзинг трассировки пути",
      "path_tracing",
      summary="Число выборок на пиксель и радиус денойзера задаются бюджетом кадра, а не максимумом качества.",
      description="Стоимость трассировки линейна по числу выборок. Вместо фиксированного качества "
                  "задаётся бюджет: разрешение внутреннего буфера, выборки на пиксель, число "
                  "отражений. Остаток сглаживается временным и пространственным денойзером.",
      problem="Полная трассировка пути с «честным» числом выборок не укладывается в бюджет кадра.",
      level="algorithm", recommended_stage="production", late_cost="low",
      impact_gpu=-2, impact_cpu=1,
      quality_impact=-1, performance_gain=0.55, implementation_cost=3, complexity=4, confidence=0.75,
      applicable_platforms=["pc_windows", "ps5", "xbox_series"],
      requires_features=["path_tracing"],
      requires_hw_features=["Hardware Ray Tracing"],
      pros=["Даёт управляемый компромисс качества и стоимости", "Позволяет удержать целевой FPS"],
      cons=["Денойзер добавляет задержку и «смаз» на движении", "Требует настройки под каждый тип сцены"],
      limitations=["Агрессивное снижение выборок даёт артефакты на тонкой геометрии"],
      application_steps=[
          "Задать бюджет RT-прохода в миллисекундах на кадр.",
          "Подобрать число выборок и разрешение внутреннего буфера под бюджет.",
          "Настроить радиус и историю денойзера по типам поверхностей.",
          "Проверить артефакты на тонкой геометрии и быстром движении камеры.",
      ],
      verification_method="Замер времени прохода и оценка шума на статичной и динамичной сцене.",
      verification_tools=["RenderDoc", "NVIDIA Nsight Graphics"],
      source_key="WIKI_PATH_TRACING"),

    M("selective_ray_traced_effects", "Выборочная трассировка отдельных эффектов",
      "ray_traced_effects",
      summary="Трассировка применяется не ко всему кадру, а к отдельным эффектам: отражениям, теням, "
              "ambient occlusion — с независимым бюджетом для каждого.",
      description="Кадр остаётся растровым, а трассировка включается там, где растровое приближение "
                  "заметно хуже. Каждый эффект получает свою долю RT-бюджета, поэтому стоимость "
                  "масштабируется по отдельности и отключается на слабом оборудовании.",
      problem="Полная трассировка пути слишком дорога, а полностью растровые отражения и тени "
              "дают заметные artefacts на блестящих и полупрозрачных поверхностях.",
      level="architecture", recommended_stage="prototype", late_cost="high",
      impact_gpu=2, impact_vram=1,
      quality_impact=1, performance_gain=0.2, implementation_cost=4, complexity=4, confidence=0.8,
      applicable_platforms=["pc_windows", "ps5", "xbox_series"],
      requires_features=["ray_traced_effects"],
      requires_hw_features=["Hardware Ray Tracing"],
      pros=["Сохраняет растровую стоимость базового кадра", "Эффекты отключаются независимо"],
      cons=["Требует поддержки двух путей рендера", "Стык растровых и трассировочных эффектов заметен"],
      limitations=["Без RT-ядер эффекты деградируют до растровых приближений"],
      application_steps=[
          "Отобрать эффекты, где растровое приближение заметнее всего.",
          "Задать каждому эффекту отдельный бюджет RT-времени.",
          "Реализовать растровый фолбэк для оборудования без RT-ядер.",
          "Проверить согласованность эффектов на стыке двух путей.",
      ],
      verification_method="Покадровое сравнение растрового и трассировочного варианта каждого эффекта.",
      verification_tools=["RenderDoc", "NVIDIA Nsight Graphics"],
      source_key="WIKI_RAY_TRACING"),

    M("rt_effect_resolution_budget", "Половинное разрешение и кэш трассировочных эффектов",
      "ray_traced_effects",
      summary="Трассировочные эффекты считаются в пониженном разрешении с временным накоплением и "
              "повторным использованием лучей между кадрами.",
      description="Стоимость трассировки растёт с числом пикселей. Отражения и AO считаются в "
                  "половинном или четвертном разрешении, результат накапливается по истории "
                  "кадров и апскейлится. Часть лучей переиспользуется между кадрами.",
      problem="Трассировочные эффекты в полном разрешении занимают основную часть бюджета GPU.",
      level="algorithm", recommended_stage="production", late_cost="medium",
      impact_gpu=-2, impact_vram=1,
      quality_impact=-1, performance_gain=0.6, implementation_cost=3, complexity=4, confidence=0.8,
      applicable_platforms=["pc_windows", "ps5", "xbox_series"],
      requires_features=["ray_traced_effects"],
      requires_hw_features=["Hardware Ray Tracing"],
      pros=["Кратно снижает стоимость RT-эффектов", "Хорошо сочетается с временным накоплением"],
      cons=["Добавляет задержку на резком движении", "Размывает мелкие детали отражений"],
      limitations=["Требует корректной инвалидации истории при смене сцены"],
      application_steps=[
          "Выделить трассировочные эффекты в отдельные буферы пониженного разрешения.",
          "Подключить временное накопление с проверкой валидности истории.",
          "Настроить апскейл и порог сброса истории.",
          "Замерять стоимость и артефакты на быстром движении.",
      ],
      verification_method="Замер времени RT-эффектов до и после и проверка «смаза» на движении.",
      verification_tools=["RenderDoc", "NVIDIA Nsight Graphics"],
      source_key="WIKI_RAY_TRACING"),

    M("dynamic_light_priority_budget", "Приоритезация и бюджет динамических источников",
      "dynamic_lighting",
      summary="Источники света получают приоритет по влиянию на кадр; источники сверх бюджета "
              "переводятся в упрощённый режим или исключаются.",
      description="Каждый кадр строится список видимых источников, им назначается приоритет по "
                  "экраной площади и близости к камере. Источники сверх бюджета теряют тени, "
                  "переходят на упрощённое затухание или отключаются.",
      problem="Сотни динамических источников без бюджета делают стоимость затенения непредсказуемой.",
      level="algorithm", recommended_stage="production", late_cost="medium",
      impact_gpu=-2, impact_cpu=-1,
      quality_impact=-1, performance_gain=0.6, implementation_cost=2, complexity=3, confidence=0.85,
      applicable_formats=["3D", "2.5D", "2D"],
      requires_features=["dynamic_lighting"],
      pros=["Делает стоимость освещения предсказуемой", "Легко настраивается под класс оборудования"],
      cons=["Возможны вспышки при переключении режимов", "Требует ручной расстановки приоритетов"],
      limitations=["Не заменяет кластеризованное отсечение, а дополняет его"],
      application_steps=[
          "Описать приоритет источников: площадь влияния, дистанция, роль в сцене.",
          "Задать бюджет числа источников с тенями и без.",
          "Реализовать плавное переключение режимов без вспышек.",
          "Замерять стоимость освещения на самой тяжёлой сцене.",
      ],
      verification_method="Замер времени прохода освещения и проверка отсутствия вспышек.",
      verification_tools=["RenderDoc", "Unity Profiler", "Unreal Insights"],
      source_key="WIKI_GLOBAL_ILLUMINATION"),

    M("light_range_attenuation_lod", "Упрощение затухания источников по дистанции",
      "dynamic_lighting",
      summary="Дальние и слабые источники переходят на упрощённую модель затухания без теней и "
              "без точной кривой спада.",
      description="Полная модель затухания с физическим спадом и тенями нужна только ближним "
                  "источникам. Сверх порога дистанции источник считается по упрощённой кривой, "
                  "а его тени заменяются запечёнными или отключаются.",
      problem="Одинаково дорогая обработка ближних и дальних источников тратит бюджет впустую.",
      level="algorithm", recommended_stage="production", late_cost="low",
      impact_gpu=-1, impact_cpu=-1,
      quality_impact=-1, performance_gain=0.45, implementation_cost=2, complexity=2, confidence=0.8,
      applicable_formats=["3D", "2.5D", "2D"],
      requires_features=["dynamic_lighting"],
      pros=["Просто внедряется", "Почти не заметно на дальнем плане"],
      cons=["Требует настройки порогов под стиль игры", "Заметно на крупных источниках у границы"],
      limitations=["Для сцен с крупными источниками на средней дистанции эффект слабее"],
      application_steps=[
          "Разделить источники по дистанции и яркости на группы.",
          "Задать упрощённую кривую затухания для дальних групп.",
          "Отключить тени у источников сверх порога.",
          "Сравнить кадры на границе перехода.",
      ],
      verification_method="Покадровое сравнение и замер стоимости освещения на эталонной сцене.",
      verification_tools=["RenderDoc", "Unity Profiler"],
      source_key="WIKI_GLOBAL_ILLUMINATION"),

    M("meshlet_pipeline_adoption", "Конвейер мешлетов на меш-шейдерах",
      "mesh_shaders",
      summary="Геометрия разбивается на мешлеты и собирается на GPU: отсечение и LOD выполняются "
              "до растеризации, без участия CPU.",
      description="Вместо фиксированного конвейера вершинных и геометрических шейдеров применяются "
                  "task/mesh-стадии. Мешлеты отсекаются и выбираются по LOD на GPU, что снимает "
                  "зависимость стоимости кадра от числа вызовов отрисовки на CPU.",
      problem="Классический конвейер упирается в число вызовов отрисовки и стоимость подготовки кадра на CPU.",
      level="architecture", recommended_stage="preproduction", late_cost="critical",
      impact_cpu=-2, impact_gpu=1,
      ...
      cons=["Требует перестройки конвейера геометрии", "Узкая база оборудования"],
      limitations=["Без поддержки меш-шейдеров нужен классический фолбэк"],
      requires_prototype=True,
      application_steps=[
          "Разбить меши на мешлеты фиксированного размера и построить цепочки LOD.",
          "Перенести отсечение и выбор LOD в task/mesh-стадии.",
          "Сохранить классический путь рендера как фолбэк.",
          "Сравнить стоимость подготовки кадра и растеризации на эталонной сцене.",
      ],
      verification_method="Замер времени подготовки кадра и геометрического прохода до и после.",
      verification_tools=["RenderDoc", "NVIDIA Nsight Graphics"],
      source_key="MS_MESH_SHADER"),

    M("gpu_meshlet_culling_budget", "Бюджет отсечения мешлетов на GPU",
      "mesh_shaders",
      summary="Отсечение мешлетов по конусу видимости, окклюзии и площади на экране с явно заданным "
              "бюджетом выживших.",
      description="Каждый мешлет проходит отсечение по frustum, затем по пирамиде окклюзии и по "
                  "минимальной экранной площади. Бюджет ограничивает число мешлетов, передаваемых "
                  "в растеризацию, что стабилизирует стоимость кадра.",
      problem="Без отсечения на GPU выигрыш меш-шейдеров сводится к нулю: растёр занимается всей геометрией.",
      level="algorithm", recommended_stage="production", late_cost="medium",
      impact_gpu=-2,
      ...
      limitations=["Окклюзионное отсечение даёт задержку на кадр"],
      application_steps=[
          "Построить границы мешлетов и иерархию для группового отсечения.",
          "Включить отсечение по конусу и минимальной площади.",
          "Добавить окклюзионное отсечение с задержкой в один кадр.",
          "Проверить popping и стоимость прохода на эталонной сцене.",
      ],
      verification_method="Замер числа выживших примитивов и времени геометрического прохода.",
      verification_tools=["RenderDoc", "NVIDIA Nsight Graphics"],
      source_key="MS_MESH_SHADER"),

    M("chunked_procedural_terrain", "Чанковая генерация процедурного ландшафта",
      "procedural_terrain",
      summary="Мир генерируется порциями вокруг наблюдателя: генерация, меши и коллизии строятся "
              "только для актуальных чанков и кэшируются.",
      description="Генератор детерминирован по координате чанка, поэтому результат воспроизводим и "
                  "кешируется. Чанки за пределами радиуса выгружаются, соседние уровни склеиваются "
                  "по границам, генерация идёт в фоновых задачах.",
      problem="Генерация всего мира целиком невозможна по времени и памяти, а генерация в главном "
              "потоке даёт заикания при движении.",
      level="architecture", recommended_stage="preproduction", late_cost="critical",
      impact_cpu=1, impact_ram=1, impact_disk=-2,
      performance_gain=0.7, implementation_cost=4, complexity=4, confidence=0.85,
      applicable_world_types=["procedural", "open_world", "sandbox"],
      requires_features=["procedural_terrain"],
      pros=["Практически неограниченный размер мира", "Детерминированность упрощает отладку и сеть"],
      cons=["Требует склейки границ чанков", "Пики генерации нужно разносить по кадрам"],
      limitations=["Не отменяет потоковую загрузку: готовая геометрия тоже занимает память"],
      application_steps=[
          "Зафиксировать размер чанка и детерминированный генератор по координате.",
          "Вынести генерацию в фоновые задачи с приоритетом по дистанции.",
          "Реализовать склейку границ между соседними уровнями детализации.",
          "Замерять время генерации чанка и отсутствие заиканий при быстром движении.",
      ],
      verification_method="Профиль главного потока при движении и замер времени генерации чанка.",
      verification_tools=["Unity Profiler", "Unreal Insights", "Профилировщик Godot"],
      source_key="WIKI_PROCEDURAL"),

    M("terrain_generation_streaming_budget", "Бюджет генерации и кэш чанков",
      "procedural_terrain",
      summary="На генерацию отводится фиксированное время кадра, а готовые чанки переиспользуются "
              "из кэша вместо повторного расчёта.",
      description="Очередь запросов на генерацию обслуживается в пределах бюджета времени кадра; "
                  "чанки, попадающие в радиус повторно, достаются из кэша. Это исключает пики "
                  "нагрузки и повторные вычисления.",
      problem="Генерация без бюджета даёт просадки кадров при быстром перемещении по миру.",
      level="algorithm", recommended_stage="production", late_cost="medium",
      impact_cpu=-2, impact_ram=1,
      performance_gain=0.55, implementation_cost=2, complexity=3, confidence=0.8,
      applicable_world_types=["procedural", "open_world", "sandbox"],
      requires_features=["procedural_terrain"],
      pros=["Убирает просадки при движении", "Кэш снижает повторные вычисления"],
      cons=["Кэш занимает память", "При нехватке бюджета мир догенерируется с задержкой"],
      limitations=["При очень высокой скорости перемещения задержка появления мира заметна"],
      application_steps=[
          "Выделить бюджет времени на генерацию в кадре.",
          "Задать политику кэша: размер, вытеснение, приоритет по дистанции.",
          "Разнести генерацию мешей и коллизий по разным кадрам.",
          "Замерять просадки при движении с максимальной скоростью.",
      ],
      verification_method="Профиль кадра при движении на максимальной скорости и замер попаданий в кэш.",
      verification_tools=["Unity Profiler", "Unreal Insights"],
      source_key="WIKI_PROCEDURAL"),
    ...
    # =====================================================================
    M("data_driven_ability_system", "Данные-ориентированная система способностей",
      "gameplay_ability_system",
      summary="Способности, модификаторы и режимы персонажа описываются данными и обрабатываются "
              "однородно: одна машина состояний вместо разрозненных проверок по коду.",
      description="Наборы способностей (нано-костюм, геройские умения, аугментации) описываются "
                  "декларативно: теги, стоимость, кулдауны, эффекты. Машина состояний одна на "
                  "всех персонажей, а модификаторы складываются по фиксированным правилам. "
                  "Это делает стоимость способности предсказуемой и измеримой.",
      problem="Разрозненные проверки способностей по коду дают непредсказуемую стоимость кадра и "
              "усложняют добавление новых умений.",
      level="architecture", recommended_stage="preproduction", late_cost="high",
      impact_cpu=-2, impact_ram=1,
      performance_gain=0.5, implementation_cost=4, complexity=4, confidence=0.7,
      applicable_formats=["3D", "2.5D", "2D"],
      requires_features=["gameplay_ability_system"],
      pros=["Предсказуемая стоимость обработки способностей", "Новые умения добавляются данными"],
      cons=["Требует перестройки боевой системы", "Ограничения модели данных заметны на нестандартных умениях"],
      limitations=["Уникальные механики всё равно потребуют собственного кода"],
      requires_prototype=True,
      application_steps=[
          "Описать модель способности: теги, стоимость, кулдаун, эффекты.",
          "Ввести единую машину состояний активации для всех персонажей.",
          "Задать правила сложения модификаторов и приоритеты.",
          "Замерять стоимость обработки активных способностей на стресс-сцене.",
      ],
      verification_method="Стресс-сцена с максимальным числом активных способностей: замер CPU.",
      verification_tools=["Unity Profiler", "Unreal Insights"],
      source_key="GPP_STATE"),

    M("ability_visual_effect_budget", "Бюджет визуальных эффектов способностей",
      "gameplay_ability_system",
      summary="Эффекты способностей (частицы, декали, свет, пост-эффекты) получают общий бюджет и "
              "приоритет по влиянию на кадр.",
      description="Каждая активная способность запрашивает визуальные эффекты. Вместо немедленного "
                  "запуска всех эффектов они складываются в очередь с приоритетом и лимитом на "
                  "одновременное число источников, частиц и декалей.",
      problem="Массовое применение способностей в бою запускает десятки тяжёлых эффектов одновременно.",
      level="algorithm", recommended_stage="production", late_cost="low",
      impact_gpu=-2, impact_cpu=-1,
      quality_impact=-1, performance_gain=0.5, implementation_cost=2, complexity=3, confidence=0.8,
      applicable_formats=["3D", "2.5D", "2D"],
      requires_features=["gameplay_ability_system"],
      pros=["Сглаживает пики в массовых боях", "Легко масштабируется по классам оборудования"],
      cons=["Часть эффектов отбрасывается", "Требует расстановки приоритетов геймдизайном"],
      limitations=["Слишком жёсткий бюджет делает способности визуально беднее"],
      application_steps=[
          "Перечислить типы эффектов и их стоимость на кадр.",
          "Задать лимиты на одновременные источники, частицы и декали.",
          "Ввести приоритет эффектов и отбрасывание сверх лимита.",
          "Замерять стоимость эффектов в сцене массового боя.",
      ],
      verification_method="Замер GPU и CPU в сцене с максимальным числом одновременных способностей.",
      verification_tools=["Unity Profiler", "Unreal Insights", "RenderDoc"],
      source_key="UE_NIAGARA"),

    M("raycast_vehicle_physics", "Физика транспорта на рейкастах подвески",
      "vehicle_simulation",
      summary="Транспорт моделируется твёрдым телом с силами в точках подвески, определяемыми "
              "рейкастами, вместо полнотельного контакта колёс.",
      description="Кузов — твёрдое тело, каждое колесо задаётся рейкастом вниз, сила подвески "
                  "рассчитывается по сжатию, сцепление — по пятну контакта. Даёт устойчивую "
                  "управляемую динамику при небольшой стоимости.",
      problem="Полнотельный контакт колёс неустойчив на высоких скоростях и дорог в симуляции.",
      level="algorithm", recommended_stage="preproduction", late_cost="high",
      impact_cpu=1,
      performance_gain=0.35, implementation_cost=3, complexity=4, confidence=0.8,
      applicable_formats=["3D", "2.5D"],
      requires_features=["vehicle_simulation"],
      pros=["Устойчивая динамика на высоких скоростях", "Небольшое число тел и контактов"],
      cons=["Требует настройки параметров под каждый тип транспорта", "Плохо передаёт деформацию покрытия"],
      limitations=["Не подходит для точной симуляции повреждений колёс"],
      application_steps=[
          "Описать кузов как твёрдое тело с точками подвески.",
          "Рассчитывать силы подвески по рейкасту и сжатию пружины.",
          "Задать модель сцепления по продольному и поперечному направлению.",
          "Замерять стоимость симуляции при максимальном числе единиц транспорта.",
      ],
      verification_method="Замер времени физического такта при максимальном числе единиц транспорта.",
      verification_tools=["Unity Profiler", "Unreal Insights"],
      source_key="WIKI_VEHICLE_DYNAMICS"),

    M("vehicle_simulation_lod", "Уровни детализации симуляции транспорта",
      "vehicle_simulation",
      summary="Дальний и неактуальный транспорт переводится на упрощённую симуляцию: кинематика "
              "по сплайну вместо полного физического расчёта.",
      description="Полный расчёт подвески и сцепления нужен только транспорту игрока и ближним "
                  "единицам. Остальные движутся по упрощённой кинематической модели с пониженной "
                  "частотой обновления, либо интерполируются между редкими тактами.",
      problem="Полная симуляция десятков единиц транспорта забирает весь бюджет физики.",
      level="algorithm", recommended_stage="production", late_cost="medium",
      impact_cpu=-2,
      quality_impact=-1, performance_gain=0.6, implementation_cost=2, complexity=3, confidence=0.8,
      applicable_formats=["3D", "2.5D"],
      requires_features=["vehicle_simulation"],
      pros=["Кратно снижает стоимость физики", "Незаметно на дальнем плане"],
      cons=["Возможны расхождения при переходе между уровнями", "Требует настройки порогов"],
      limitations=["В сетевой игре переходы должны быть согласованы между клиентами"],
      application_steps=[
          "Определить уровни: полная симуляция, упрощённая, кинематика.",
          "Задать пороги по дистанции и видимости.",
          "Реализовать плавный переход без рывка траектории.",
          "Замерять стоимость физики при максимальном числе единиц.",
      ],
      verification_method="Замер времени физического такта и проверка плавности переходов.",
      verification_tools=["Unity Profiler", "Unreal Insights"],
      source_key="WIKI_VEHICLE_DYNAMICS"),

    M("behaviour_tree_update_budget", "Деревья поведений с бюджетом обновления",
      "advanced_npc_ai",
      summary="Поведение NPC описывается деревом или конечным автоматом, а частота и глубина "
              "обновления ограничиваются бюджетом кадра.",
      description="Логика NPC выносится из кода в данные: узлы дерева или состояния автомата. "
                  "Обновление распределяется по кадрам по приоритету — ближние и заметные NPC "
                  "обновляются чаще, дальние реже и по упрощённому поддереву.",
      problem="Сотни NPC с индивидуальной логикой в главном потоке дают непредсказуемые просадки.",
      level="architecture", recommended_stage="preproduction", late_cost="high",
      impact_cpu=-2,
      performance_gain=0.6, implementation_cost=3, complexity=4, confidence=0.75,
      applicable_formats=["3D", "2.5D", "2D"],
      requires_features=["advanced_npc_ai"],
      pros=["Предсказуемая стоимость ИИ", "Поведение правится данными, а не кодом"],
      cons=["Требует перестройки ИИ", "Редкое обновление даёт задержку реакции"],
      limitations=["Для стелс-систем задержка реакции критична и бюджет нужно задавать мягче"],
      application_steps=[
          "Описать поведение NPC деревом или автоматом в данных.",
          "Разделить узлы по стоимости и глубине обновления.",
          "Задать бюджет обновлений на кадр и приоритет по заметности.",
          "Замерять время ИИ и задержку реакции на стресс-сцене.",
      ],
      verification_method="Стресс-сцена: замер времени ИИ и задержки реакции NPC.",
      verification_tools=["Unity Profiler", "Unreal Insights"],
      source_key="WIKI_BEHAVIOR_TREE"),

    M("npc_perception_budget", "Бюджет восприятия NPC",
      "advanced_npc_ai",
      summary="Проверки видимости и слуха выполняются по квоте: ограниченное число запросов в кадр "
              "с приоритетом по близости и важности NPC.",
      description="Восприятие — самая дорогая часть индивидуального ИИ. Запросы складываются в "
                  "очередь с приоритетом, на кадр выполняется фиксированное число лучей и "
                  "проверок слуха, остальные переносятся на следующие кадры.",
      problem="Массовые проверки видимости для каждого NPC на каждом кадре забирают бюджет симуляции.",
      level="algorithm", recommended_stage="production", late_cost="medium",
      impact_cpu=-2,
      quality_impact=-1, performance_gain=0.55, implementation_cost=2, complexity=3, confidence=0.8,
      applicable_formats=["3D", "2.5D", "2D"],
      requires_features=["advanced_npc_ai"],
      pros=["Сглаживает стоимость восприятия", "Приоритет отдаётся значимым NPC"],
      cons=["Добавляет задержку обнаружения", "Требует настройки квоты под сцену"],
      limitations=["В стелс-сценах малая квота делает ИИ невнимательным"],
      application_steps=[
          "Выделить проверки восприятия из логики NPC в отдельную систему.",
          "Ввести очередь запросов с приоритетом по близости и состоянию.",
          "Задать квоту лучей и проверок на кадр.",
          "Замерять задержку обнаружения и стоимость восприятия.",
      ],
      verification_method="Замер времени восприятия и задержки обнаружения на стресс-сцене.",
      verification_tools=["Unity Profiler", "Unreal Insights"],
...
EXTRA_LINKS: dict[str, dict[str, tuple[str, str, str]]] = {
    ...
    "full_path_tracing_pipeline": {
        "unreal": ("ue_lumen", "partial", "Lumen на аппаратной трассировке близок к PT, но остаётся гибридом с растровыми кэшами."),
        "unity": ("u_srp", "partial", "Path Tracing в HDRP есть, но рассчитан на офлайн-режим и узкий набор сцен."),
        "godot": ("g_gi", "missing", "Полной трассировки пути нет: только SDFGI и VoxelGI."),
        "custom": ("c_render_graph", "alternative", "Собственный проход трассировки пути в графе рендера."),
    },
    "path_tracing_sample_denoiser_budget": {
        "unreal": ("ue_lumen", "partial", "Число выборок и радиус денойзера задаются консольными переменными Lumen."),
        "unity": ("u_srp", "partial", "Накопление и денойзер HDRP с настройкой числа кадров истории."),
        "godot": ("g_gi", "missing", "Денойзера для трассировки пути нет."),
        "custom": ("c_render_graph", "alternative", "Денойзер и бюджет выборок пишутся в собственном проходе."),
    },
    "selective_ray_traced_effects": {
        "unreal": ("ue_lumen", "direct", "Отражения, тени и AO на трассировке включаются независимо."),
        "unity": ("u_srp", "partial", "Трассировочные эффекты HDRP настраиваются по объёмам и слоям."),
        "godot": ("g_gi", "missing", "Выборочной аппаратной трассировки нет."),
        "custom": ("c_render_graph", "alternative", "Отдельные RT-проходы в собственном графе рендера."),
    },
    "rt_effect_resolution_budget": {
        "unreal": ("ue_lumen", "partial", "Разрешение трассировки и накопление задаются масштабом Lumen."),
        "unity": ("u_srp", "partial", "Половинное разрешение эффектов настраивается в HDRP."),
        "godot": ("g_gi", "missing", "Настраиваемого бюджета трассировочных эффектов нет."),
        "custom": ("c_render_graph", "alternative", "Масштаб буферов и накопление задаются в собственном проходе."),
    },
    # --- Динамическое освещение ---------------------------------------------
    "dynamic_light_priority_budget": {
        "unreal": ("ue_significance", "direct", "Significance Manager задаёт приоритет объектов и источников."),
        "unity": ("u_quality", "partial", "Число пиксельных источников задаётся настройками качества."),
        "godot": ("g_visibility", "partial", "Приоритет задаётся дистанциями и слоями видимости вручную."),
        "custom": ("c_manual", "alternative", "Приоритезация источников реализуется кодом проекта."),
    },
    "light_range_attenuation_lod": {
        "unreal": ("ue_lod", "partial", "Дальность и тени источника задаются в его настройках."),
        "unity": ("u_light_probes", "partial", "Дальние источники заменяются зондами и упрощённым затуханием."),
        "godot": ("g_shadows", "partial", "Дистанции теней и затухание задаются по каждому свету."),
        "custom": ("c_manual", "alternative", "Пороги и упрощённая кривая задаются кодом проекта."),
    },
    # --- Меш-шейдеры ---------------------------------------------------------
    "meshlet_pipeline_adoption": {
        "unreal": ("ue_nanite", "partial", "Nanite использует кластеризованную геометрию, но свой растеризатор, а не открытый конвейер меш-шейдеров."),
        "unity": ("u_srp", "missing", "Конвейера меш-шейдеров в URP/HDRP нет."),
        "godot": ("g_mesh_lod", "missing", "Меш-шейдеров нет, только LOD и MultiMesh."),
        "custom": ("c_render_graph", "alternative", "Task/mesh-стадии подключаются в собственном графе рендера."),
    },
    "gpu_meshlet_culling_budget": {
        "unreal": ("ue_nanite", "partial", "Кластерное отсечение встроено в Nanite, но не настраивается отдельно."),
        "unity": ("u_srp", "missing", "Отсечения мешлетов на GPU нет."),
        "godot": ("g_occluder", "partial", "Окклюзионные отсекатели вместо мешлетов."),
        "custom": ("c_render_graph", "alternative", "Отсечение мешлетов реализуется в compute-проходе."),
    },
    # --- Процедурный мир -----------------------------------------------------
    "chunked_procedural_terrain": {
        "unreal": ("ue_world_partition", "partial", "World Partition даёт чанки, но генератор пишется самостоятельно."),
        "unity": ("u_addressables", "partial", "Чанки как отдельные сцены и адресуемые ресурсы."),
        "godot": ("g_bg_loading", "partial", "Чанки грузятся фоновым загрузчиком, генератор — кодом."),
        "custom": ("c_streaming", "alternative", "Собственная подсистема стриминга и генерации чанков."),
    },
    "terrain_generation_streaming_budget": {
        "unreal": ("ue_world_partition", "partial", "Приоритет и радиус загрузки задаются настройками партиции."),
        "unity": ("u_jobs", "direct", "Генерация в C# Job System с распределением по кадрам."),
        "godot": ("g_threads", "direct", "Генерация в потоках с приоритетом по дистанции."),
        "custom": ("c_job_system", "direct", "Собственная система задач с бюджетом на кадр."),
    },
    # --- Геймплейные и симуляционные подсистемы ------------------------------
    "data_driven_ability_system": {
        "unreal": ("ue_gas", "direct", "Gameplay Ability System: атрибуты, эффекты, теги и кулдауны из данных."),
        "unity": ("u_dots", "partial", "ECS-каркас задаёт данные-ориентированную основу, система способностей — кодом."),
        "godot": ("g_visibility", "missing", "Готовой системы способностей нет, собирается из узлов и ресурсов."),
        "custom": ("c_ecs", "alternative", "Система способностей поверх собственного ECS."),
    },
    "ability_visual_effect_budget": {
        "unreal": ("ue_niagara", "partial", "Niagara Significance и масштабируемость ограничивают эффекты."),
        "unity": ("u_vfxgraph", "partial", "Лимиты и приоритеты эффектов настраиваются в VFX Graph."),
        "godot": ("g_gpu_particles", "partial", "Лимиты частиц задаются настройками систем частиц."),
        "custom": ("c_manual", "alternative", "Бюджет эффектов реализуется кодом проекта."),
    },
    "raycast_vehicle_physics": {
        "unreal": ("ue_vehicles", "direct", "Chaos Vehicles: подвеска на рейкастах, сцепление и привод."),
        "unity": ("u_quality", "missing", "Встроенной физики транспорта нет, только WheelCollider и свои модели."),
        "godot": ("g_physics_server", "partial", "VehicleBody3D даёт упрощённую модель, тонкая настройка ограничена."),
        "custom": ("c_manual", "alternative", "Модель подвески и сцепления пишется кодом."),
    },
    "vehicle_simulation_lod": {
        "unreal": ("ue_vehicles", "partial", "LOD симуляции собирается из настроек и Significance Manager."),
        "unity": ("u_quality", "partial", "Пороги упрощения задаются настройками качества и скриптами."),
        "godot": ("g_physics_server", "partial", "Переключение уровня симуляции — кодом на PhysicsServer."),
        "custom": ("c_job_system", "alternative", "Уровни симуляции задаются в собственной системе задач."),
    },
    "behaviour_tree_update_budget": {
        "unreal": ("ue_behavior_tree", "direct", "Деревья поведений с настройкой интервалов обновления."),
        "unity": ("u_navmesh", "partial", "Поведение собирается из NavMeshComponents и собственных стейт-машин."),
        "godot": ("g_navigation", "partial", "Поведение на узлах и скриптах, бюджет обновления — кодом."),
        "custom": ("c_ecs", "alternative", "Деревья или автоматы поверх собственного ECS."),
    },
    "npc_perception_budget": {
        "unreal": ("ue_behavior_tree", "partial", "Восприятие через AI Perception с настройкой интервалов."),
        "unity": ("u_navmesh", "missing", "Готовой системы восприятия нет, пишется кодом."),
        "godot": ("g_visibility", "partial", "Видимость через VisibilityNotifier и слои, квоты — кодом."),
        "custom": ("c_manual", "alternative", "Очередь запросов восприятия реализуется кодом проекта."),
    },
}
...
EXTRA_CONFLICTS: list[dict] = [
    {
        ...
        "conflict_type": "complement", "severity": 1,
        "description": "Атлас повышает эффективность батчинга скелетной 2D-анимации, но не является "
                       "обязательным условием: скелетная анимация работает и без общего атласа, "
                       "просто с большим числом вызовов отрисовки.",
        "resolution": "Планировать атласы вместе с переходом на скелетную анимацию; отсутствие "
                      "атласа не делает решение неприменимым.",
        "source_key": "WIKI_ATLAS",
    },
    {
        "a_code": "lightmap_2d_baking", "b_code": "post_effect_selective",
        "conflict_type": "complement", "severity": 1,
        "description": "Освещение и постобработка — независимые проходы: каждый учитывается своей "
                       "стоимостью. Дополнительного бонуса только от совместного выбора нет.",
        "resolution": "Применять совместно для 2D-проектов с большим числом источников света, "
                      "учитывая каждый проход отдельно.",
        "source_key": "UNITY_LIGHTMAPPER",
    },
    {
        "a_code": "skeletal_2d_deform", "b_code": "sprite_sheet_compression",
        "conflict_type": "risk", "severity": 1,
        "description": "Риск качества при конкретном сжатии: потери заметнее на деформируемых "
                       "частях тела, чем на готовых кадрах покадровой анимации. Это не "
                       "несовместимость скелетной анимации и сжатия как таковых.",
        "resolution": "Для скелетной анимации использовать менее агрессивное сжатие и проверить "
                      "артефакты на деформируемых частях.",
        "source_key": "WIKI_MIPMAP",
    },
    {
        "a_code": "crowd_2d_instancing", "b_code": "agent_update_budget",
        "conflict_type": "risk", "severity": 1,
        "description": "Батчинг и расписание AI — разные виды работы. Не доказано, что снижение "
                       "частоты обновления агентов всегда остаётся незаметным при массовой "
                       "отрисовке одним батчем.",
        "resolution": "Проверить реакцию агентов при сниженной частоте обновления на сценах с "
                      "максимальным числом агентов.",
        "source_key": "UE_SIGNIFICANCE",
    },
    # --- Альтернативные методы: баланс набора --------------------------------
    {
        "a_code": "deterministic_lockstep", "b_code": "multithreaded_physics_jobs",
        "conflict_type": "risk", "severity": 3,
        "description": "Параллельная физика требует детерминированного порядка и математики: иначе "
                       "результат зависит от планировщика и lockstep расходится. Проблема условная, "
                       "а не запрет всех многопоточных реализаций физики.",
        "resolution": "Обеспечить детерминированный порядок шагов и фиксированную математику либо "
                      "параллелить только независимые read-only проходы.",
        "source_key": "GAFFER_TIMESTEP",
    },
    {
        "a_code": "ml_frame_generation", "b_code": "client_prediction_reconciliation",
        "conflict_type": "risk", "severity": 2,
        "description": "Генерация кадров добавляет задержку и может давать визуальные артефакты "
                       "при пересборке состояния. Генерация и предсказание не являются "
                       "универсально несовместимыми алгоритмами.",
        "resolution": "Измерить задержку ввода и артефакты реконсиляции; для competitive-режимов "
                      "генерацию отключать.",
        "source_key": "WIKI_DLSS",
    },
    {
        ...
        "conflict_type": "complement", "severity": 1,
        "description": "Оба решения уменьшают разные источники пауз: прогрев устраняет компиляцию "
                       "шейдеров, асинхронная загрузка — ожидание чтения. Ни одно из них не "
                       "гарантирует отсутствия всех пауз и не даёт постоянного бонуса FPS.",
        "resolution": "Применять совместно для миров со стримингом, проверяя пики времени кадра "
                      "на реальных переходах.",
        "source_key": "UNITY_SHADERLOAD",
    },
    {
        "a_code": "quality_tier_scalability", "b_code": "hardware_raytraced_gi",
        "conflict_type": "complement", "severity": 1,
        "description": "Scalability выбирает реальный набор настроек, а не удешевляет уже "
                       "рассчитанную трассировку: отдельной скидки к стоимости RT только от "
                       "наличия тиров нет.",
        "resolution": "Привязать RT-пресеты к верхним тирам и проверять на min-spec тира; стоимость "
                      "каждого тира считать отдельно.",
        "source_key": "UNITY_QUALITY",
    },
    {
        "a_code": "planar_reflection_budget", "b_code": "screen_space_gi",
        "conflict_type": "unknown", "severity": 1,
        "description": "Экранное GI — непрямое освещение, а не экранные отражения (SSR): утверждение, "
                       "будто экранное GI уже считает плоские отражения, ошибочно. Запрета пара "
                       "не образует, но совместная стоимость двух проходов не подтверждена "
                       "измерением отдельно.",
        "resolution": "Проверить суммарную стоимость плоских отражений и экранного GI на "
                      "representative-сцене; каждый проход учитывается своей стоимостью.",
        "source_key": "UE_LUMEN",
    },
    {
        "a_code": "temporal_upscaling", "b_code": "hardware_raytraced_gi",
        "conflict_type": "complement", "severity": 2,
        "description": "Upscaler, denoiser и ray reconstruction — разные проходы со своей "
                       "стоимостью. Совместимость апскейлера с трассировкой не даёт ещё одной "
                       "скидки поверх стоимости реконструкции.",
        "resolution": "Проектировать RT-пресеты в паре с апскейлером, учитывая стоимость "
                      "денойзера и реконструкции отдельно.",
        "source_key": "WIKI_DLSS",
    },
    {
        "a_code": "tiled_clustered_light_culling", "b_code": "depth_prepass_early_z",
        "conflict_type": "risk", "severity": 2,
        "description": "Часть алгоритмов light culling использует вход глубины, но обязательный "
                       "полный depth pre-pass для всей категории не требуется: это условие "
                       "конкретной реализации, а не свойство подхода.",
        "resolution": "Определить по выбранной реализации, нужен ли проход глубины, и учитывать его "
                      "стоимость отдельно от классификации источников.",
        "source_key": "CLUSTERED_SHADING",
    },
]
...
CONFLICTS: list[dict] = [
    {
        "a_code": "lightmap_atlas_baking", "b_code": "hardware_raytraced_gi", "conflict_type": "alternative",
        "severity": 3,
        "description": "Условная альтернатива для одной области освещения: запечённый свет и "
                       "полностью динамическое освещение решают одну задачу разными способами. "
                       "Смешанная сцена допустима — статичные объекты могут оставаться запечёнными, "
                       "поэтому универсального запрета пара не образует.",
        "resolution": "Определить область освещения, где применяется каждый механизм, и зафиксировать "
                      "это до производства контента; без такого разделения выбрать одну схему.",
        "source_key": "WIKI_LIGHTMAP",
    },
    {
        "a_code": "gpu_procedural_placement", "b_code": "lightmap_atlas_baking", "conflict_type": "risk",
        "severity": 2,
        "description": "Запечённые данные существуют только для заранее известной геометрии, поэтому "
                       "размещаемые в рантайме объекты остаются без лайтмапа. Ограничение "
                       "распространяется на процедурную часть сцены: остальные объекты могут "
                       "оставаться запечёнными.",
        "resolution": "Использовать зонды освещённости для процедурных объектов и определить, какая "
                      "часть сцены остаётся запечённой.",
        "source_key": "UNITY_LIGHTPROBES",
    },
    {
    ...
    {
        "a_code": "motion_matching", "b_code": "animation_compression", "conflict_type": "risk",
        "severity": 2,
        "description": "Риск относится к конкретной степени сжатия: искажённые позы ухудшают точность "
                       "поиска в базе движений. Универсальной несовместимости motion matching и "
                       "сжатия анимаций нет.",
        "resolution": "Проверить точность подбора на выбранном уровне сжатия и исключить из сильного "
                      "сжатия движения, участвующие в базе motion matching.",
        "source_key": "WIKI_SKINNING",
    },
    {
        "a_code": "static_shadow_caching", "b_code": "hardware_raytraced_gi", "conflict_type": "risk",
        "severity": 2,
        "description": "Кэш теней рассчитан на неизменное освещение, а трассировка пересчитывает его "
                       "заново: выигрыш кэша зависит от доли статичных источников. Разные виды "
                       "света и теней могут сосуществовать, поэтому запрета пара не образует.",
        "resolution": "Кэшировать только тени статических источников и явно задать правила "
                      "инвалидации кэша при изменении освещения.",
        "source_key": "UE_VSM",
    },
    {
        "a_code": "virtual_texturing", "b_code": "heightmap_compression", "conflict_type": "complement",
        "severity": 1,
        "description": "Совместимые форматы тайлов дополняют виртуальное текстурирование: сжатие "
                       "повышает эффективность кэша и снижает трафик подкачки. Выигрыш зависит от "
                       "формата, размера кэша и стоимости декодирования и не даётся автоматически.",
        "resolution": "Проверить формат тайлов, размер кэша и стоимость декодирования; совместимость "
                      "не означает дополнительной скидки к уже рассчитанному эффекту каждого решения.",
        "source_key": "UE_VIRTUALTEXTURING",
    },
    {
        "a_code": "gpu_particle_simulation", "b_code": "particle_pooling", "conflict_type": "complement",
        "severity": 1,
        "description": "CPU-пулинг и GPU-симуляция затрагивают разную работу: пулинг устраняет "
                       "аллокации при создании систем, симуляция переносит расчёт на GPU. "
                       "Абстрактные скидки не перемножаются.",
        "resolution": "Применять совместно, учитывая стоимость каждого решения отдельно.",
        "source_key": "WIKI_OBJECTPOOL",
    },
    {
        "a_code": "agent_update_budget", "b_code": "crowd_instancing_impostors", "conflict_type": "risk",
        "severity": 1,
        "description": "Снижение частоты обновления дальних агентов совместимо с их упрощённой "
                       "отрисовкой, но независимость экономии AI и рендера внутри одной группы "
                       "не доказана: заметность задержки требует проверки.",
        "resolution": "Проверить на representative-сцене, что сниженная частота обновления остаётся "
                      "незаметной на фоне упрощённых представлений.",
        "source_key": "UE_SIGNIFICANCE",
    },
    {
        "a_code": "network_relevancy_priority", "b_code": "delta_compression_state", "conflict_type": "complement",
        "severity": 1,
        "description": "Relevancy и дельта-кодирование уменьшают разные части трафика: первое "
                       "сокращает набор передаваемых объектов, второе — размер каждого обновления. "
                       "Нужна единая модель набора и байтов; сравнение и упаковка добавляют CPU.",
        "resolution": "Применять совместно, оценивая трафик в байтах и отдельные CPU-расходы на "
                      "сравнение и упаковку.",
        "source_key": "WIKI_DELTA",
    },
]

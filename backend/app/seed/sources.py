...
`date` — дата публикации, если она объявлена издателем. Для постоянно
обновляемой документации и справочных страниц такой даты не существует, и
пишется `n/a`: это объявленное отсутствие, а не пропуск. Прежнее значение
`2025-01-01` подставлялось как заглушка и читалось как настоящая дата
...
SOURCES: dict[str, dict[str, str]] = {
    "UE_DISTANCE_SHADOWS": {
        ...
        # Страница обновляемой документации движка: издатель даты публикации не
        # объявляет, поэтому по правилу модуля здесь `n/a`. Прежнее значение
        # `2026-09-07` — дата проверки ссылки, а не публикации, — читалось как
        # настоящая дата публикации и противоречило соседним страницам Epic.
        "date": "n/a",
    },
    # --- Unreal Engine -------------------------------------------------
    "UE_NANITE": {
    ...
    "UNITY_SHADERLOAD": {
        ...
        # путь `Manual/ShaderLoadTimeOptimization.html` отдаёт 404 (проверено
        # пробой). Страница «Shader loading» — та же тема у того же вендора,
        # ответ 200. Замена адреса, а не источника.
        "url": "https://docs.unity3d.com/6000.0/Documentation/Manual/shader-loading.html",
        "date": "n/a",
    },
...
# Даты публикации у этих страниц нет: это обновляемая документация и страницы
# SDK. Дата обращения (2026-09-09) в поле даты публикации читалась как факт об
# источнике, а не как день проверки доступности.
SOURCES.update({key: dict(title=title, url=url, date='n/a') for key, (title, url) in TECHNICAL_SOURCES.items()})

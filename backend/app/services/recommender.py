...
def detect_risks(
    ...
    # Риск «срок разработки против трудоёмкости» убран вместе с полем
    # `deadline_weeks`. Он сравнивал срок с числом решений по константе
    # «4 недели на трудоёмкое решение», которая нигде не обоснована, а посчитанной
    # трудоёмкости в модели больше нет — её подсистема вырезана. Оставлять
    # утверждение «внедрение за этот срок маловероятно» без расчёта нельзя.

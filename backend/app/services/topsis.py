...
#: Порог различимости коэффициента близости.
#
# Критерии TOPSIS — экспертные баллы, а веса проверяются на устойчивость
# возмущением ±10%. Разница коэффициентов меньше порога не означает
# преимуществa: она лежит внутри точности исходных оценок. По итогам аудита
# размах оценок лидера на 58 проектах составил 0.016 — меньше порога, поэтому
# строгий топ-1 в такой ситуации выдавать нельзя.
EQUIVALENCE_TOLERANCE = 0.02


def equivalence_group(scores: list[float], tolerance: float = EQUIVALENCE_TOLERANCE) -> list[bool]:
    """Отметить альтернативы, неотличимые от лидера по коэффициенту близости.

    Возвращает список флагов той же длины, что `scores`: True у альтернатив,
    отстающих от лучшей меньше чем на `tolerance`. Группа равнозначных
    показывает, что строгого победителя нет, и выбор внутри неё — вопрос
    соответствия задаче, а не рейтинга.
    """
    if not scores:
        return []
    best = max(scores)
    return [score >= best - tolerance for score in scores]

...
_GUIDANCE: dict[str, dict] = {
    "concept": {
        "summary": "На концепте проще сравнить архитектурные варианты; фактическая стоимость зависит от прототипов и уже созданных компонентов.",
        "rework": [],
        "suggestions": [
            ("target_hw", "Определить целевую конфигурацию",
             "Зафиксировать целевые платформу, разрешение и частоту кадров: от них зависит "
             "бюджет кадра и ограничения технических решений."),
            ("fix_architecture", "Заложить архитектурные решения",
             "Схема потоковой загрузки, модель памяти, путь рендера и сетевая топология "
             "полезно проверить сейчас; позднее их смена может затронуть больше готовых компонентов."),
    ...
    "preproduction": {
        "summary": "До массового производства проверьте архитектуру и форматы данных. Стоимость последующих изменений зависит от накопленной реализации.",
        "rework": [],
        "suggestions": [
    ...
    "prototype": {
        ...
        "suggestions": [
            ("measure", "Измерить, а не оценить",
             ...
             "их эффект для вашего проекта ещё не измерен."),
        ...
        "warnings": [
            ("architecture_cost", "Архитектура дорожает",
             "Проверьте, какие интерфейсы и материалы прототипа затронет смена архитектуры."),
        ],
    },
    "production": {
        ...
        "warnings": [
            ("rework", "Проверить последствия архитектурных правок",
             "Сопоставьте новое решение с реализованной основой: часть компонентов может остаться без изменений."),
        ],
    },
    "alpha": {
        "summary": "На альфе учитывайте готовые подсистемы и зависимости. Оценка перехода от реализованной корзины уточняет общий риск стадии.",
        "rework": ["architecture"],
        "suggestions": [
            ...
            ("algorithms", "Взять алгоритмические решения",
             "Проверяйте алгоритмические варианты на выявленном узком месте; стадия не определяет величину выигрыша."),
        ],
        "warnings": [
            ("architecture_rework", "Проверить объём архитектурных изменений",
             "Решения уровня архитектуры с высокой и критической ценой позднего внедрения "
             "остаются в списке; сохранность интерфейсов, данных и ассетов определяет фактический объём работ."),
            ("content_freeze", "Избегать решений, влияющих на контент",
             "Сопоставьте переделку ассетов с текущим планом наполнения уровней."),
        ],
    },
    "beta": {
        ...
        "warnings": [
            ...
            ("architecture_rework", "Проверить стоимость архитектурных правок",
             "Пометка указывает на риск позднего внедрения. Она не доказывает необходимость переписывать весь проект."),
        ],
    },
    "release": {
        "summary": "При подготовке релиза оцените объём изменения, регрессионные проверки и возможность отката.",
        "rework": ["architecture"],
        "suggestions": [
            ...
            ("fallback", "Предусмотреть откат",
             "Для изменения подготовьте путь отката и оцените время исправления возможных регрессий."),
            ("baseline", "Зафиксировать базовую версию",
             "Сохранить текущий набор решений как базу: сравнение с ней покажет, что именно "
             "изменил патч."),
        ],
        "warnings": [
            ("no_content_change", "Проверить изменения контента перед выпуском",
             "Для производственных и архитектурных правок оцените пересборку материалов, совместимость данных и объём тестирования."),
    ...
    "post_release": {
        ...
        "suggestions": [
            ...
            ("patch_baseline", "Сравнивать патч с базовой версией",
             "Зафиксировать реализованную корзину; сравнивать с ней новый набор и стоимость перехода."),
        ...
        "warnings": [
            ...
            ("architecture_rework", "Оценить архитектурный переход",
             "Смена архитектуры может потребовать переноса данных и кода. Сохранённая основа помогает отделить повторно используемые компоненты от переделываемых."),
        ],
    },
}
...
def rework_note(method: Method, stage: str) -> str:
    ...
    return (
        f"На стадии «{DevStage(stage_code(stage)).label}» проверить объём переработки: "
        f"уровень «{SolutionLevel(method.level).label}» с "
        f"{LateCost(method.late_cost).label} ценой позднего внедрения может затронуть готовые "
        "материалы. Это экспертный риск; фактический объём зависит от исходной реализации и сохранности интерфейсов."
    )

...
точка цепочки маркеров (от начальной ревизии до `meta01_research_meta` при
полном соответствии), затем выполняется обычный `upgrade head`. Неизвестная
структура — отказ с явной ошибкой, а не догадка.
"""
...
#: Ревизия, добавившая доказательный слой (`evidence_sources` и прочие таблицы).
#: Это НЕ голова цепочки: после неё идут `fix01_userdef`, `sync01_model_indexes`
#: и `meta01_research_meta`. Здесь она нужна только как маркер: унаследованная
#: база, у которой уже есть `evidence_sources`, штампуется сюда, а остаток
#: цепочки применяет обычный `upgrade head`. Прежнее имя `HEAD_REVISION` и
#: комментарий «голова цепочки» описывали состояние до `meta01`: значение
#: голове не соответствовало, и следующая правка легко штамповала бы базу
#: несуществующей головой.
EVIDENCE_LAYER_REVISION = "dss_evidence01"
...
LEGACY_MARKER_CHAIN: tuple[tuple[str, str, str], ...] = (
    ...
    ("evidence_sources", "id", EVIDENCE_LAYER_REVISION),
    # Колонки исследовательского слоя. В цепочке они идут последними: база,
    # созданная приложением до этой ревизии, не имеет ни одной из них и
    # штампуется на прежнюю голову, а остаток применяет обычный upgrade.
    # Без этих маркеров сверка структуры видела бы лишние колонки модели и
    # отказывалась штамповать унаследованную базу (штамп запрещён).
    ("methods", "implementation_variants", "meta01_research_meta"),
    ("methods", "required_data_and_tools", "meta01_research_meta"),
    ("conflicts", "basis", "meta01_research_meta"),
    ("dependency_edges", "basis", "meta01_research_meta"),
)
